...
public class L_C_SOGBOFA_H2 extends Policy {

	static Logger log = LogManager.getLogger(L_C_SOGBOFA_H2.class);

	public L_C_SOGBOFA_H2() {
		super();
	}

	public L_C_SOGBOFA_H2(String instance_name) {
		super(instance_name);
		Global.ifLift = true;
	...
	ArrayList<PVAR_INST_DEF> bufferConformantActs = new ArrayList<>();
	...
	TreeExp BuildF(State s) throws Exception {
		...
		for (PVAR_NAME p : s._alActionNames) {
			...
			if (Policy._extraEffects.containsKey(p)) {
				...
				while (it.hasNext()) {
					...
					ArrayList<TYPE_NAME> constraintsTypeName = (ArrayList<TYPE_NAME>) pair.getKey();
		...
		for (; h < maxDepth; h++) {
			...
			//}
			//actions
			ArrayList<PVAR_INST_DEF> trajeActs = new ArrayList<PVAR_INST_DEF>();
	...
	public void SampleAction(State s, ArrayList<ArrayList<PVAR_INST_DEF>> conformantActs, ArrayList<Double> useNum) throws EvalException{
		...
		for(int h = 0; h < maxVarDepth; h ++) {
			ArrayList<PVAR_INST_DEF> finalAction = new ArrayList<PVAR_INST_DEF>();
			int counter = 0;
			for(PVAR_NAME p: s._alActionNames){
	...
	public ArrayList<PVAR_INST_DEF> getActions(State s) throws EvalException {
		...
		// a non-noop actions trajectory
		if(conformantActs.size() > 1 && conformantActs.get(1).size()>1){
			log.debug("layer 1 : {}", conformantActs.get(1));
			bufferConformantActs = conformantActs.get(1); // save actions
			log.debug("return conformantActs(0) {} ", conformantActs.get(0));
		...
		if(conformantActs.get(0).size() == 0 && bufferConformantActs.size()>1){
			...
			for(PVAR_INST_DEF act : bufferConformantActs){
				if (act._oValue instanceof Double && (Double)act._oValue>=0.8){
					act._oValue = true;
					ret.add(act);
				}else if ((act._oValue instanceof Boolean && (Boolean) act._oValue == true)){
					ret.add(act);
				}else{
					act._oValue = false;
				}
			}
			log.debug("return bufferConformantActs {} ", ret);
			bufferConformantActs.clear();
			return ret;
		}
		log.debug("return conformantActs(0) {} ", conformantActs.get(0));
		return conformantActs.get(0);
