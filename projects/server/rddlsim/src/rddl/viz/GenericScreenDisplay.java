...
import static rddl.competition.Server.IS_STATISTICS;
...
public class GenericScreenDisplay extends StateViz {
	...
	private int numberOfGroundStates;
	private int numberOfGroundActions;
	...
	public String getStateDescription(State s) {
				...
				try {
					...
					if(IS_STATISTICS){
						if (e.getKey().equals("states")) {
							numberOfGroundStates += gfluents.size();
						}
						if (e.getKey().equals("action")) {
							numberOfGroundActions += gfluents.size();
						}
					}
		...
		if(IS_STATISTICS){
			System.out.println("NumberOfGroundStates: " + numberOfGroundStates);
			System.out.println("NumberOfGroundActions: " + numberOfGroundActions);
			System.exit(1);
		}
