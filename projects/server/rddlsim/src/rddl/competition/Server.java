...
public class Server implements Runnable {
    ...
    private static int DEFAULT_NUM_ROUNDS = 1;
    private static int DEFAULT_HORIZON = 10;
    private static int TIME_PER_STEP = 2; // s
    private static long DEFAULT_TIME_ALLOWED = (long)(1000.0 * TIME_PER_STEP * DEFAULT_HORIZON * DEFAULT_NUM_ROUNDS); // milliseconds = 18 minutes
//    private static long DEFAULT_TIME_ALLOWED = 1000 * 1000 * 100 * DEFAULT_NUM_ROUNDS; // milliseconds = 18 minutes
    ...
    private static String LOG_INFO_FILE = "rddl-info";
    ...
    // Variables for CPU time measurement.
    public static int type_of_time = 0;
    public static String group_name = "ippc_test";
    ...
    public static void main(String[] args) {
        StateViz state_viz = null;

        if(IS_STATISTICS){
            state_viz = new GenericScreenDisplay(true);
        }else{
            state_viz = new NullScreenDisplay(false);
        }
        ...
        try {
            ...
            System.out.println("");
            System.out.println("using RDDL input files from args[0]: " + SERVER_FILES_DIR);

            if ( args.length > 1) {
                port = Integer.valueOf(args[1]);
                System.out.println("setting port to args[1]: " + port);
            } else {
                System.out.println("using default port at: " + port);
            }
            ServerSocket socket1 = new ServerSocket(port);
 
            if (args.length > 2) {
                DEFAULT_NUM_ROUNDS = Integer.valueOf(args[2]);
                System.out.println("setting number of rounds to args[2]: " + DEFAULT_NUM_ROUNDS);
            } else {
                System.out.println("using default number of rounds: " + DEFAULT_NUM_ROUNDS);
            }

            int rand_seed = -1;
            if ( args.length > 3) {
                rand_seed = Integer.valueOf(args[3]);
                System.out.println("setting seed to args[3]: " + rand_seed);
            } else {
                rand_seed = DEFAULT_SEED;
                System.out.println("setting seed to random value: " + rand_seed);
            }

            if (args.length > 4) {
                if (args[4].equals("1")) {
                    INDIVIDUAL_SESSION = true;
                    System.out.println("enabling individial session due to args[4]");
                } else {
                    System.out.println("disabling individial session due to args[4]");
                }
            } else {
                System.out.println("individual session is disabled due to default setting");
            }

            if (args.length > 5) {
                if (args[5].equals("0")) {
                    USE_TIMEOUT = false;
                    System.out.println("no timeout is used due to args[5]");
                } else {
                    USE_TIMEOUT = true;
                    DEFAULT_TIME_ALLOWED = Integer.valueOf(args[5]) * 1000;
                    System.out.println("timeout is used due to args[5]: " + DEFAULT_TIME_ALLOWED);
                }
            } else {
                System.out.println("default timeout is used: " + DEFAULT_TIME_ALLOWED);
            }

            if (args.length > 6) {
                LOG_FILE = args[6] + "/logs";
                System.out.println("due to args[6], logs are written to: " + LOG_FILE);
            } else {
                System.out.println("logs are written to default file at: " + LOG_FILE);
            }
            ...
            if (args.length > 7) {
                ...
                if (args[7].equals("1")) {
                    ...
                    System.out.println("enabling execution monitoring due to args[7]");
                } else {
                    System.out.println("disabling execution monitoring due to args[7]");
                }
            } else {
                System.out.println("execution monitoring is disabled due to default setting");
            }

            if (args.length > 8) {
                type_of_time = Integer.parseInt(args[8]);
                System.out.println("type of time is set to args[8]: " + type_of_time);
            } else {
                System.out.println("using default type of time: " + type_of_time);
            }

            if (args.length > 9) {
                group_name = args[9];
                if (type_of_time == 1) {
                    System.out.println("setting cgroup name to args[9]: "+ group_name);
                }
                else {
                    System.out.println("setting planner name to args[9]: "+ group_name);
                }
            } else {
                System.out.println("cgroup and planner name are not used");
            }

            if (args.length > 10) {
                state_viz = (StateViz)Class.forName(args[10]).newInstance();
                System.out.println("setting visualization to args[10]: " + args[10]);
            }
    ...
    public void run() {
        ...
        try {
            ...
            long start_time = getTime(group_name);
            ...
            InputSource isrc = readOneMessage(isr, timeAllowed, start_time);
            ...
            StringBuffer sb = new StringBuffer();
                sb.append(requestedInstance+"\n")
                .append("Session finished successfully: " + clientName+"\n")
                .append("Time left: " + getRemainingTime(group_name, timeAllowed, start_time, false)+"\n")
                .append("Number of simulations: " + numSimulations+"\n")
                .append("Number of runs: " + numRounds+"\n")
                .append("Accumulated reward: " + (accum_total_reward)+"\n")
                .append("Average reward: " + (accum_total_reward / numRounds)+"\n");

            System.out.print(sb.toString());

            writeInfoToLog(sb.toString());
    ...
    public void writeInfoToLog(String msg) throws IOException {
        BufferedWriter bw = new BufferedWriter(new FileWriter(LOG_INFO_FILE + "-" + this.port + ".log" , true));
        bw.write(msg);
        bw.newLine();
        bw.flush();
        bw.close();
    ...
    public static synchronized InputSource readOneMessage(InputStream isr, long timeAllowed, long start_time) {
    ...
    public static long getTime(String group_name) throws java.io.IOException {
        if (type_of_time == 0) return System.currentTimeMillis();
        Process process = Runtime.getRuntime().exec(new String[] {"cgget", "-nv", "-r", "cpuacct.usage", group_name});
